...
//raceinfo:{'starttime'(string),'startlon'(real),'startlat'(real),'endlon'(real),'endlat'(real)}
//tracks:list(object)
...
function splicer_straightAndRouteEff(raceinfo, tracks) {
    ...
    var raceStartPoint = turf.point([raceinfo['startlon'], raceinfo['startlat']]);
    var raceEndPoint = turf.point([raceinfo['endlon'], raceinfo['endlat']]);
...
//track data object:{'lon'(real),'lat'(real),'utc'(string),'distance'(real)}
/////////////////
//return:list(object)
//object:{racerecordid(string),straightspeed(list to string),routeefficiency(list to string)}
function splicerStraightAndRouteEffWithTime(raceinfo, tracks,minutes) {
    ...
    //make timeslicerlist by race_totaldistance:50km is 1h; 
    const raceStartTime = new Date(raceinfo['starttime']);
    console.log('raceStartTime',raceStartTime);
    // console.log(race_totaldistance);
    // const timesSlicer = Math.floor(race_totaldistance / 50);
    // let timesSlicer_list = [];
    // for (var i = 1; i < timesSlicer; i++) {
    ...
    //get the last arriving time of all
    let lastArrivingTime = new Date(tracks[0]['data'][tracks[0]['data'].length-1]['utc']);
    for(var i=1;i<tracks.length;i++)
    {
        var arrivalTime = new Date(tracks[i]['data'][tracks[i]['data'].length-1]['utc']);
        if(lastArrivingTime.getTime()<arrivalTime.getTime())
        {
            lastArrivingTime = arrivalTime;
        }
    }
    console.log('lastArrivingTime',lastArrivingTime);
    //get slicer list by minutes and start time and last arrival time
    var timeGap = (lastArrivingTime.getTime() - raceStartTime.getTime())/(1000*60); //trans milisecond to minutes
    let timesSlicer_list = [];
    for (var i = 1; i < timeGap/minutes; i++) {
        var tmp_date = new Date();
        tmp_date.setTime(raceStartTime.getTime() + i * minutes * 60 * 1000);
        timesSlicer_list.push(tmp_date);
    }
    console.log(timeGap, timesSlicer_list);

    //make track dashbaord by timesSlicer_list
    let rowForDashboard_list = [];
    for (var i = 0; i < tracks.length; i++) {
        var rowForDashboard = {};
        rowForDashboard['racerecordid'] = tracks[i]['racerecordid'];
        const [straightspeed, routeefficiency] = dashboard(tracks[i]['data']);
        rowForDashboard['straightspeed'] = straightspeed;
        rowForDashboard['routeefficiency'] = routeefficiency;
        rowForDashboard_list.push(rowForDashboard);
    ...
    function dashboard(track) {
        ...
        // }
        for (var i = 1; i < track.length - 1; i++) {
            var curTime = new Date(track[i]['utc']);
            ...
            if (curTime == picker_time) {
                ...
                var nextTime = new Date(track[i + 1]['utc']);
                if (curTime < picker_time && nextTime > picker_time) {
                    // console.log(curTime,picker_time,(total_flyDistance/1000).toFixed(3));
                    // console.log(curTime,nextTime,picker_time);
                    var cur_dif = picker_time - curTime;
        ...
        for (var i = 0; i < pickedPoints.length; i++) {
            ...
            var curTime = new Date(pickedPoints[i]['utc']);
            var time_diff = curTime.getTime() - raceStartTime.getTime();
            // console.log(time_diff);
            time_diff = (time_diff / 1000) / 3600;
            var straightspeed = (race_totaldistance - leftdistance) / time_diff;
        ...
        // console.log(dash_realtimeRouteEfficiency);
        // return [str_dash_straightSpeed, str_dash_realtimeRouteEfficiency];
        return [dash_straightSpeed, dash_realtimeRouteEfficiency];
    }
}
...
//input
//params:raceinfo(object),tracks(list)
//raceinfo:{'racename'(string),'starttime'(string),'startlon'(real),'startlat'(real),'endlon'(real),'endlat'(real)}
//tracks:list(object)
...
//input
//params:raceinfo(object),tracks(list)
//raceinfo:{'raceid'(string),'starttime'(string),'startlon'(real),'startlat'(real),'endlon'(real),'endlat'(real)}
//tracks:list(object)
...
function track_make_withRaceEndpoint(raceinfo, tracks) {
    ...
    for (var i = 0; i < tracks.length; i++) {
        ...
        var endPoint = turf.point([raceinfo['endlon'], raceinfo['endlat']]);
        var fake_lastPoint = {
            'lon': raceinfo['endlon'],
            'lat': raceinfo['endlat'],
...
module.exports.splicerStraightAndRouteEffWithTime = splicerStraightAndRouteEffWithTime;
